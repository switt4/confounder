if config["MASK_FILE"]:
	rule run_fsl_glm_mask:
		input:
			...
			design_mat = join(feat_dir,'task-{task}',subj_sess_dir,'confound-{confound_name}','run-{run}',f'design.mat')
			mask_file = config["MASK_FILE"]
		temp:
            param_estimate_file_list = join(feat_dir,f'param_estimate_file_list.txt')
